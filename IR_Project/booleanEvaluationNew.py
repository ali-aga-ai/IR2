...
# Import Boolean retrieval
from booleanQuery import load_index_and_chunk_table, query_boolean_index

# --- Load the same benchmark_qna as in evaluation.py ---
benchmark_qna = {
...
def to_boolean_query(query):
    # Remove punctuation, split to words, join with OR
    import re
    words = re.findall(r'\w+', query)
    return ' OR '.join(words)

def boolean_chunks_to_context(chunks, max_chunks=25):
    # Concatenate up to max_chunks chunks for LLM context
    context = ""
    for i, chunk in enumerate(chunks[:max_chunks]):
        context += f"{i}th Retrieved chunk: {chunk['text']}... source: {chunk['chunk_id']}\n"
    return context
...
def get_llm_response(query, context):
    m = [
        {
            "role": "system",
            "content": """You are a strict factual assistant for BITS Pilani policy documents. Follow these rules:
1. Answer ONLY using information from the retrieved segments below. Never use prior knowledge.
2. For numerical/date/fact queries:
   - If an exact match exists, provide a direct, concise answer with the source.
   - If only partial information is available, state "Partial information: [quote]" with the source.
   - If no relevant information is found, say "No relevant information found in documents."
   - Keep the response direct and to the point, but you may include a brief supporting excerpt if helpful.
3. For general, descriptive, or procedural queries:
   - Synthesize a clear and informative answer using only the retrieved segments.
   - Be concise but allow for some explanation or context where appropriate.
   - Summarize or paraphrase as needed, but do not add information not present in the segments.
   - Always cite the source document for each fact or statement.
4. Do not infer, assume, or combine information across sources.
5. If you hallucinate information, students may face academic consequences.
...
def evaluate_boolean_model():
    model = SentenceTransformer('all-MiniLM-L6-v2')
    results = []
    for question, expected_answer in benchmark_qna.items():
        print(f"Question: {question}")
        print(f"Expected Answer: {expected_answer}")

        # Convert to boolean query
        boolean_query = to_boolean_query(question)
        print(f"Boolean Query: {boolean_query}")

        # Retrieve chunks
        chunks = query_boolean_index(inverted_index, chunk_id_table, boolean_query)
        context = boolean_chunks_to_context(chunks, max_chunks=25)

        # LLM response
        hypothesis_answer = get_llm_response(question, context)
        print("Model Response:", hypothesis_answer)

        # BERTScore
        emb1 = model.encode(expected_answer, convert_to_tensor=True)
        emb2 = model.encode(hypothesis_answer, convert_to_tensor=True)
        bert_score = util.pytorch_cos_sim(emb1, emb2).item()
    ...
    df.to_excel("boolean_output.xlsx", index=False)
    print("Results saved to boolean_output.xlsx")

if __name__ == "__main__":
    evaluate_boolean_model()